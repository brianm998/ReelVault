...
package com.reelvault.data.server
 ...
 * `REELVAULT_LISTENING_ON=127.0.0.1:N` line from stdout to discover which
...
class ServerLauncher {
     ...
     *  1. `REELVAULT_CORE_BIN` environment variable (full path to executable).
     *  2. Sibling-to-jar location: `<jar dir>/resources/reelvault-core[.exe]`.
     *  3. Cargo dev builds: `<project root>/core/target/release/reelvault-core`
     *     then `…/target/debug/reelvault-core`.
    ...
    fun locateBinary(): File? {
        // 1. Env override
        System.getenv("REELVAULT_CORE_BIN")?.takeIf { it.isNotBlank() }?.let { p ->
            val f = File(p)
            if (f.isFile && f.canExecute()) return f
        }

        val binaryName = if (isWindows()) "reelvault-core.exe" else "reelvault-core"
    ...
    fun launch(preferredPort: Int? = null, dbPath: String? = null): Listening? {
        val bin = locateBinary() ?: run {
            logger.error("reelvault-core binary not found")
            return null
        }
        ...
        val sentinel = "REELVAULT_LISTENING_ON="
        ...
        }, "reelvault-core-stdout").apply { isDaemon = true }.start()
