...
/// `VIDEOROOM_LISTENING_ON=127.0.0.1:N` line from stdout so the client knows
/// where to connect.
///
...
final class ServerLauncher {
    ...
    ///   1. `VIDEOROOM_CORE_BIN` env var.
    ///   2. Inside the app bundle: `Resources/videoroom-core`.
    ///   3. Project-root dev builds (search up from cwd for a `core/target` dir).
    ///   4. `/usr/local/bin/videoroom-core`, `~/.cargo/bin/videoroom-core`.
    /// Returns `nil` if nothing was found — caller should surface a clear error.
    func locateBinary() -> URL? {
        let fm = FileManager.default

        if let env = ProcessInfo.processInfo.environment["VIDEOROOM_CORE_BIN"], !env.isEmpty {
        ...
        if let resURL = Bundle.main.resourceURL?
            .appendingPathComponent("videoroom-core"),
        ...
        for _ in 0..<6 {
            let release = dir.appendingPathComponent("core/target/release/videoroom-core")
            if fm.isExecutableFile(atPath: release.path) { return release }
            let debug = dir.appendingPathComponent("core/target/debug/videoroom-core")
        ...
        for path in [
            "/usr/local/bin/videoroom-core",
            "/opt/homebrew/bin/videoroom-core",
            "\(NSHomeDirectory())/.cargo/bin/videoroom-core",
        ] {
    ...
    func launch(preferredPort: Int = 0, dbPath: String? = nil) async -> Listening? {
        guard let bin = locateBinary() else {
            NSLog("ServerLauncher: videoroom-core binary not found")
            return nil
        }
        ...
        let sentinel = "VIDEOROOM_LISTENING_ON="
