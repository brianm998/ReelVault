; NSIS installer script for the ReelVault Core daemon.
;
...
Section "Install"
    SetOutPath "$INSTDIR"
    File "${PKG_DIR}\reelvault-core.exe"
    File "${PKG_DIR}\reelvault-cli.exe"
    ...
    WriteRegStr   HKLM "${REG_UNINSTALL}" "DisplayIcon"     "$INSTDIR\reelvault-core.exe,0"
...
Section "Uninstall"
    ...
    Delete "$INSTDIR\reelvault-core.exe"
    Delete "$INSTDIR\reelvault-cli.exe"
